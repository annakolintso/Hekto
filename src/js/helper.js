export const createDropdown = () => {
  ...
  const dropdownLocStore = localStorage.getItem('dropdown');
  const parsedData = (dropdownLocStore && JSON.parse(dropdownLocStore) || {});
  dropdowns && dropdowns.forEach((dropdown) => {
    ...
    items && items.forEach((item) => {
      ...
      item.addEventListener('click', () => {
        ...
        const activeItem = dropdown.querySelector('.dropdown__item.active');
